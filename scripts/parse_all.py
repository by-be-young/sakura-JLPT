# 通用解析：从红宝书4卷提取全部等级词条
# N1=卷1全部 | N2=卷2全部 | N3=卷3全部 | N4=卷4[0:3343] | N5=卷4[3344:]
import docx, re, json

F1 = r'C:\Users\34166\Desktop\超值白金版·红宝书大全集  新日本语能力考试N1-N5文字词汇详解_1-145.docx'
...
F4 = r'C:\Users\34166\Desktop\超值白金版·红宝书大全集  新日本语能力考试N1-N5文字词汇详解_436-580.docx'

def paras_of(path):
    d = docx.Document(path)
    return [p.text.strip() for p in d.paragraphs if p.text.strip()]

p1 = paras_of(F1); p2 = paras_of(F2); p3 = paras_of(F3); p4 = paras_of(F4)

# 分区（段落级）
N4_MARK = 3344  # 卷4 N5构成标记位置
zones = {
    'N1': p1,
    'N2': p2,
    'N3': p3,
...
def clean_circle(s):
    ...
    for p in pats:
        s = re.sub(p, lambda m: CIRC.get(m.group(1), m.group(0)), s)
...
MANUAL_FIX = {
    'アウ卜': 'アウト', '才一ケス卜ラ': 'オーケストラ', '力ッ卜': 'カット',
    'サ一クル': 'サークル', 'スク一ル': 'スクール', '夕亻卜ル': 'タイトル',
    '二ーズ': 'ニーズ', '一ゲン': 'バーゲン', 'ベストセラー': 'ベストセラー',
    '~編': 'へん', '~逼': 'へん', 'マ一ク': 'マーク', '毛一夕一': 'モーター',
    'ュ二一ク': 'ユニーク', 'エアメール': 'エアメール', '才 一 ケス卜ラ': 'オーケストラ',
}
...
def parse_entry(t):
    ...
    if m_kana:
        ...
        if re.fullmatch(r'[a-zA-Z]+', kana_raw):
            ...
            kana = fix_kana(kana_raw); kanji = word
    else:
        word = head
        w2 = re.sub(r'[⓪①②③④⑤⑥⑦⑧⑨⑩]', '', word)
        if KANA_OK.match(w2):
...
out = {}
for level, paras in zones.items():
    entries = []
    for t in paras:
        if t.startswith('□'):
            e = parse_entry(t)
    ...
    out[level] = good
    print(f'{level}: 解析 {len(entries)} / kana合法 {len(good)} / 异常 {len(bad)}')
    for e in bad[:5]:
        print('   bad:', e['kanji'][:15], e['kana'][:15], e['pos'])

with open('all_zones.json', 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('已写 all_zones.json')
