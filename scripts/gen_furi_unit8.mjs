// 为听解 Unit8 数据生成振假名（ruby）与读音（kana）
// 用法：node scripts/gen_furi_unit8.mjs
import fs from 'fs'
...
const DIR = 'D:/日语自学网站/src/data/listening/unit8'
...
fs.writeFileSync(DIR + '/words.js', '// 听解 Unit8 自然・科学・健康 · 词汇板块（聴いてみよう＜自然・科学・健康に関する語彙＞）\n' +
  '// 来源：绿宝书《新日本语能力考试N2听解（详解+练习）》Part1 基础编 Unit8 书页84-86\n' +
...
fs.writeFileSync(DIR + '/questions.js', '// 听解 Unit8 自然・科学・健康 · 题目板块（問題351-400）\n' +
  '// 来源：绿宝书《新日本语能力考试N2听解（详解+练习）》Part1 基础编 Unit8 书页87-89\n' +
...
for (const part of knowledge.parts) {
  if (part.kind === 'card') {
    ...
    // 短汉字词被 isChinese 误判为中文时的强制注音
    const FORCE_FURI = { '一方': '<ruby>一方<rp>(</rp><rt>いっぽう</rt><rp>)</rp></ruby>' }
    for (const p of part.pairs || []) {
...
fs.writeFileSync(DIR + '/knowledge.js', '// 听解 Unit8 自然・科学・健康 · 补充知识板块（聴解の基礎知識8＜話の展開と会話表現＞）\n' +
  '// 来源：绿宝书《新日本语能力考试N2听解（详解+练习）》Part1 基础编 Unit8 书页90-92\n' +
  '// - parts: 翻页部分；kind: card 表现卡片（no/pattern/meaning/desc/lines 对话示例）\n' +
