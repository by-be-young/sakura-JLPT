# -*- coding: utf-8 -*-
"""
将新解析的 N3 数据合入 src/data/grammar.js，替换旧 N3，保留 N5/N4/N2/N1（含其 furi）。
"""
import json, re, os

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JS = os.path.join(ROOT, 'src', 'data', 'grammar.js')
NEW = os.path.join(ROOT, 'src', 'data', '_n3_new.json')
...
idx = next(i for i, l in enumerate(levels) if l['id'] == 'N3')
old_pts = sum(len(u['points']) for u in levels[idx]['units'])

with open(NEW, encoding='utf-8') as f:
    new_n3 = json.load(f)
new_pts = sum(len(u['points']) for u in new_n3['units'])
print(f'旧 N3: {old_pts} 点 -> 新 N3: {new_pts} 点')
levels[idx] = new_n3

out = '// 自动生成：N5/N4/N3 由 scripts/gen_grammar_n5.py、gen_grammar_n4.py、gen_grammar_n3.py 从蓝宝书各分册解析，N2/N1 由旧脚本生成，请勿手改。\n' \
