// 为N1读解题库生成振假名：articleFurigana / 每题 stemFurigana / 难句 sentenceFurigana
// 用法：node scripts/gen_reading_furigana_n1.mjs
import fs from 'fs'
...
const target = 'D:/日语自学网站/src/data/reading-n1.js'
const fileUrl = 'file:///' + target.replace(/\\/g, '/')
const mod = await import(fileUrl)
const readings = mod.readingN1
...
const lines = []
lines.push('// 读解题库（N1）：橙宝书《新日本语能力考试N1读解》· 基础编')
...
lines.push('// - 振假名由 scripts/gen_reading_furigana_n1.mjs 自动生成，请勿手改')
lines.push('export const readingN1 = [')
for (const r of readings) {
